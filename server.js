  ...
  resolvers: {
    ...
    Mutation: {
      async addContact(_, { contact }) {
        let newContact = { ...contact, id: uid(), creationDate: new Date(Date.now()).toISOString(), modifiedDate: new Date(Date.now()).toISOString() };
        await db
          .get("contacts")
          .push(newContact)
          .write();
      ...
      async updateContact(_, { contact }) {
        await db
          .get("contacts")
          .find({ id: contact.id })
          .assign({ ...contact, modifiedDate: new Date(Date.now()).toISOString() })
          .write();
